		...
		for SIM in 1; do
			...
			for TAG in '_20240226_data_run23696'; do
				...
				for RWZ in 0 1; do
					...
					ZUP=$(( $ZLO + 2 ))
					NAME="condor_${ZCUT}${SCALE}${SUB}${MINE}${DAT}${DAT}${TAG}${TAG2}${RWZ}${ZLO}${ZUP}.sh"
					echo "#!/bin/bash" > $NAME
					...
					echo "root -b -q \"build_hists.C(${DAT},${DAT},${ZCUT},${SCALE},${SUB},${MINE},\\\"${TAG}\\\",\\\"${TAG2}\\\",${RWZ},${ZLO},${ZUP})\"" >> $NAME
					chmod +x $NAME
					SUBN="condor_${ZCUT}${SCALE}${SUB}${MINE}${DAT}${DAT}${TAG}${TAG2}${RWZ}${ZLO}${ZUP}.sub"
